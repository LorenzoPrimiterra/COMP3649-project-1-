...
Pipeline:
=====================
    (1) Parser.hs          reads and validates the input file.

    (2) Intermediate.hs    stores the three-address instruction sequence.

    (3) Liveness.hs        computes live_before and live_after sets.

    (4) Interference.hs    builds the interference graph and assigns registers.

    (5) CodeGen.hs         translates IR into assembly language.

    (6) Target.hs          stores and formats the generated assembly.

    (7) <input>.s          output assembly file.
...
- readFile is lazy, so Main.hs forces full evaluation via evaluate and
  showIntermediateCode to ensure parse errors surface at the right sstage
